...
echo "=== Deploy Go Backend to EC2 ==="
echo ""
echo "Copy and run this script on your EC2 instance (3.148.227.249):"
echo ""
echo "================================================"
cat << 'DEPLOY_SCRIPT'
#!/bin/bash
...
mkdir -p ~/macal-backend && cd ~/macal-backend
...
func main() {
	...
	r.HandleFunc("/api/v1/inspections", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
...
# Build the backend
go build -o macal-backend main.go

# Create systemd service
sudo tee /etc/systemd/system/macal-backend.service > /dev/null <<'SERVICE'
[Unit]
Description=MACAL Inventory Backend
After=network.target

[Service]
Type=simple
User=ec2-user
WorkingDirectory=/home/ec2-user/macal-backend
ExecStart=/home/ec2-user/macal-backend/macal-backend
Restart=always
RestartSec=10
...
sudo systemctl enable macal-backend
sudo systemctl start macal-backend

# Check status
sleep 2
sudo systemctl status macal-backend
...
echo "================================================"
echo ""
echo "Frontend is already built and pointing to EC2 backend."
echo "Frontend files are in: frontend/dist/"
echo ""
echo "To complete deployment:"
echo "1. Run the script above on your EC2 instance"
echo "2. Upload frontend/dist/ to S3 or serve it from EC2"
echo ""
